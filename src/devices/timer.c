...
/* combination of waiting thread, start time, time to wake. */
struct waiting_elem
  {
...
{
  /*waiting_elem of current thread */
  struct waiting_elem of_curr;
...
timer_interrupt (struct intr_frame *args UNUSED)///
{
  ...
  /* need to check whether there's thread to wake up */
  for(elem = list_begin(&wait_list); elem != list_end(&wait_list); elem = list_next(elem)){
    w1 = list_entry(elem, struct waiting_elem, elem);
